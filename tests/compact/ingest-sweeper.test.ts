...
describe("runIngestSweep", () => {
  ...
  it("embeds chunks, upserts to qdrant, updates point ids, and marks completed on success", async () => {
    ...
    expect(qdrant.upsert).toHaveBeenCalledWith("memory_chunks_v1", {
      points: [expect.objectContaining({ id: `chunk:${chunk.id}` })],
    });
  ...
  it("schedules retry with exponential backoff on transient failure (attempt < maxAttempts)", async () => {
    ...
    const qdrant = {
      upsert: vi.fn().mockRejectedValue(new Error("Qdrant 503")),
    };
  ...
  it("is idempotent: re-upserting the same chunks with the same point ids is safe", async () => {
    ...
    // Point id is deterministic from chunk id
    const [firstCall, secondCall] = qdrant.upsert.mock.calls;
    expect(firstCall[1].points[0].id).toBe(secondCall[1].points[0].id);
  });
});
