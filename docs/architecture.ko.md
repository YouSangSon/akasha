...
마이그레이션은 `src/db/migrations/` 의 001-006 번호로 부트스트랩 시 적용.
...
`src/embedding/embedding-factory.ts` 가 provider 선택:

- `openai` (기본) → `src/embedding/openai-embeddings.ts`,
  `text-embedding-3-small`, 1536-dim.
- `local` → `src/embedding/local-embeddings.ts`, 결정론적 SHA-256 해싱
  → 384-dim 벡터. 외부 호출 없음; CI / air-gapped / 오프라인 가능.

provider는 부트스트랩 시 `EMBEDDING_PROVIDER` 에서 선택, 프로세스 lifetime
동안 `services.embeddings` 에 보관. provider 변경 시 reindex (`reindex_memory`
도구) 필요 — dimension 과 컨텐츠 의미 다름.
