...
search_memory →  search tool  →  retrieveMemory  →  embeddings.embed →  transformers / openai / local
                                 ...
                                 (lexical)         repository.searchMemory → Postgres scope keyword 후보
...
마이그레이션은 `src/db/migrations/` 에 위치. 현재 범위는 `001-009` 이며,
런너는 부트스트랩 시 `001` 부터 `009` 까지 적용합니다 (모두 idempotent,
