...
응답에 `WWW-Authenticate` discovery hint를 포함합니다. 이는 discovery
레이어일 뿐입니다. 서버는 여전히 bearer token을 `Authorization` 헤더로만
받고, `MEMORY_API_TOKENS` 에 대해 검증합니다.
...
- **OAuth token validation은 아직 없음.** Protected-resource discovery는
  호환성 foundation이지 authorization server 통합 전체가 아닙니다. Third-party
  API token을 pass-through 하지 말고, audience 검증 token verifier가 생기기
  전까지는 Akasha-issued/static bearer token을 계속 사용하세요.
